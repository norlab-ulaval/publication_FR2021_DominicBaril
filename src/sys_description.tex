...
The angular velocity command can then be computed as follows
...
It is only the distance from the end goal that was used in order to prevent the robot from finishing it's course abruptly.
Thus, the commanded linear velocity $v_x$ is computed as follows
...
At each time step, a new exponential function and command $\bm u$ are computed using the control laws shown in~\autoref{eq:exp_PD} and~\autoref{eq:exp_lin_vel}, using the robot's 2D pose $x_{2D}$ and reference trajectory $\bm x_{ref}$ as input.
