...
In addition, lidar sensors typically make the assumption that the lidar frame $\lidarf$ is static during each scan, which is incorrect for applications such as \ac{LTR}.
Hence, a point cloud de-skewing approach was added to our system in order to a de-skewed point cloud $\hat{\readpc}$.
First proposed by~\citet{Zhang2014}, such algorithms use the the high-frequency odometry to correct the location of each point before registration.
