...
We first transform $\readpc$ to $\robotf$ through \transform{\lidarf}{\robotf}.
Thus, the \ac{ICP} algorithm estimates the transform $_{\robotf}^{\mapf}\bm{T}$ by minimizing an error function $\text{error}(\readpc, \refpc)$:

\begin{equation}
	_{\robotf}^{\mapf}\bm{\hat{T}} = \argmin_{\bm T} (\text{error} \left(\readpc, \refpc, _{\robotf}^{\mapf}\bm{\check{T}} \right))
\end{equation}

where $_{\robotf}^{\mapf}\bm{\check{T}}$ is the prior transformation, estimated via the \ac{IMU} and wheel odometry.
...
% There are multiple matches in Q for every point in P (7)
...
The resulting points can then be appended to $\refpc$ and the resulting transform \transform{\robotf}{\mapf} can be used to express the 3D robot pose in $\mapf$.
...
As specified above, the \ac{ICP} algorithm requires high-frequency odometry to produce a prior estimate $_{\robotf}^{\mapf}\bm{\check{T}}$.
...
Each command is propagated with the robot orientation between scans, yielding $_{\robotf}^{\mapf}\bm{\check{T}}$ at a frequency of \SI{100}{Hz}.
